...
master = mavutil.mavlink_connection('tcp:192.168.2.3:5763')
...
#        ratio_colormap = cv2.applyColorMap(ratio_normalized.astype(np.uint8), cv2.COLORMAP_JET)
        ...
        if time_diff >= 0.5:
            ...
            gps_info[f"image_{curr_time}"]={'lon':msg['lon']*1e-7,
                                      'lat':msg['lat']*1e-7,
                                      'alt':msg['relative_alt']/1000}
            prev_time = curr_time
            filename1 = f"{save_path1}image_{curr_time}.jpg"
            filename2 = f"{save_path2}image_{curr_time}.jpg"
...
def load_mission(cords):
    ...
    master.mav.mission_count_send(master.target_system, master.target_component,len(cords)+3,0)
    ack_message('MISSION_REQUEST')
    master.mav.mission_item_int_send(master.target_system, master.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0 , 0, alt)    
    ack_message('MISSION_REQUEST')      
    master.mav.mission_item_int_send(master.target_system, master.target_component, 1, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_TAKEOFF, 0, 0, 0, 0, 0, 0, 0 , 0, alt)          
    ack_message('MISSION_REQUEST')   
    for i,cord in enumerate(cords):
        print(f'sending waypoint {i+2}')
        master.mav.mission_item_int_send(master.target_system, master.target_component,i+2,mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT_INT,mavutil.mavlink.MAV_CMD_NAV_WAYPOINT,0,1,0,2,20,math.nan,int(cord['lat'] * 1e7),int(cord['lng'] * 1e7),alt,mavutil.mavlink.MAV_MISSION_TYPE_MISSION) 
        ack_message('MISSION_REQUEST')
    print('sending RTL command')
    master.mav.mission_item_int_send(master.target_system, master.target_component, len(cords)+2, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_RETURN_TO_LAUNCH, 0, 0, 0, 0, 0, 0, 0 , 0, 0)          
    ack_message('MISSION_ACK')
...
while True:
    ...
    cords=connection.recv(10845760).decode()
